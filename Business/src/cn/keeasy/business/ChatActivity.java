...
import cn.keeasy.business.adapter.ChatAdapter;
import cn.keeasy.business.adapter.FaceAdapter;
import cn.keeasy.business.mod.CurrContactMod;
import cn.keeasy.business.mod.SendMsgMod;
...
public class ChatActivity extends BaseActivity implements OnTouchListener {
	...
	void initData() {
		...
		sendmod = new SendMsgMod(ChatActivity.this, this);
	...
	private void sendMsg() {
		if (msg.getText().toString().length() > 0) {
			sendmod.mSendMessage(Sources.USERBEAN.userId,
					Sources.CURRENTIC.contactAccount, "text", msg.getText()
							.toString(), Sources.USERBEAN.shopIC);
			msg.setText("");
		} else {
	...
	protected void onResume() {
		ccmod.mGetMessageInfo(Sources.USERBEAN.shopIC,
				Sources.CURRENTIC.contactAccount, Sources.USERBEAN.userId);
		ccmod.mStart();
		super.onResume();
		recorder.restore();
	}
