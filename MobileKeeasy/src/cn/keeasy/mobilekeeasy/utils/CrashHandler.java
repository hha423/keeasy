...
import cn.keeasy.mobilekeeasy.BaseActivity;
...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	public void init(Context ctx) {
		mContext = ctx;
		sp = ctx.getSharedPreferences("userInfo", 0);
		sendMod = new SendMsgMod(ctx, null, sp);
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				PhoneState.showToast(mContext,
						"很抱歉,程序出现异常,即将退出！\r\n错误信息已反馈至服务器,待工程师查阅解决！");
	...
	private void postReport(File file) {
		...
		if (file != null && file.exists()) {
			sendMod.mSendMidMessage(PM.fileToByte(file), "log");
		}
	}
	...
	private String saveCrashInfoToFile(Throwable ex) {
		...
		try {
			...
			String fileName = "crash-" + mTime + CRASH_REPORTER_EXTENSION;
		...
		try {
			...
			if (pi != null) {
				mDeviceCrashInfo.put(VERSION_NAME,
						pi.versionName == null ? "not set" : pi.versionName+"&-KeeasyMobile");
